...
class tag2pix(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.end_epoch + 1):
            ...
            for iter, (original_, sketch_, skeleton_, iv_tag_, cv_tag_) in enumerate(tqdm(self.train_data_loader, ncols=80)):
                if iter >= max_iter:
                    break

                if self.gpu_mode:
                    sketch_, original_, skeleton_, iv_tag_, cv_tag_ = sketch_.to(self.device), original_.to(self.device), skeleton_.to(self.device), iv_tag_.to(self.device), cv_tag_.to(self.device)
                ...
                G_f, _ = self.G(sketch_, skeleton_, feature_tensor, cv_tag_)
                if self.gpu_mode:
                ...
                G_f, G_g = self.G(sketch_, skeleton_, feature_tensor, cv_tag_)
    ...
    def visualize_results(self, epoch, fix=True):
        ...
        # test_data_loader
        original_, sketch_, skeleton_, iv_tag_, cv_tag_ = self.test_images
        ...
        with torch.no_grad():
            ...
            if self.gpu_mode:
                original_, sketch_, skeleton_, iv_tag_, cv_tag_, feature_tensor = original_.to(self.device), sketch_.to(self.device), skeleton_.to(self.device), iv_tag_.to(self.device), cv_tag_.to(self.device), feature_tensor.to(self.device)
    ...
    def get_test_data(self, test_data_loader, count):
        test_count = 0
        original_, sketch_, skeleton_, iv_tag_, cv_tag_ = [], [], [], [], []
        for orig, sket, skeleton, ivt, cvt in test_data_loader:
